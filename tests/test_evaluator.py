...
class TestEvaluator(unittest.TestCase):
        ...
        for attrs in invalid_attributes:
            with self.subTest(attrs=attrs):
                with self.assertRaises(TypeError):
                    evaluator.evaluate(config, "test_flag", attrs)
    ...
    def test_record_evaluations(self):
        ...
        evaluator.evaluate(cfg, "a", {"id": "10", "__now": 1000})
        evaluator.evaluate(cfg, "a", {"id": "12", "__now": 1001})
        evaluator.evaluate(cfg, "a", {"id": "12", "__now": 1002})
        evaluator.evaluate(cfg, "b", {"id": "18", "__now": 1003})
        evaluator.evaluate(cfg, "a", {"id": "10", "__now": 1004})
        evaluator.evaluate(cfg, "a", {"id": "18", "__now": 1005})
        evaluator.evaluate(cfg, "a", {"id": "19", "__now": 1006})
        evaluator.evaluate(cfg, "b", {"id": "88", "__now": 1007, "some_other_attr": 6})

        evals = evaluator.pop_evaluations()

        evals.sort(key=lambda x: (x.timestamp, x.flag))
        eval_tuples = [(e.timestamp, e.flag, e.attributes["id"], e.reason, e.variant) for e in evals]
    ...
    def test_pop_evaluations_when_disabled(self):
        ...
        evaluator.evaluate(cfg, "a", {"id": "test_id"})
        evaluator.evaluate(cfg, "a", {"id": "test_id2"})
